...
class SolveTSPUsingACO:
    ...
    def run(self):
        ...
        print('Sequence : <- {0} ->'.format(' - '.join(str(12341+self.labels[i]) for i in self.global_best_tour)))
...
def main(_colony_size,_steps):    
     #_colony_size = 55
     #_steps = 50
     _nodes = [(random.uniform(-90, 90), random.uniform(-90, 90)) for _ in range(0, customerno-1)]
     ...
     maxminfinalroute=[]
     maxminfinalroute,maxminpartial=max_min.run()
     max_min.plot()
     ...
     print("MAXMIN")
     costmaxmin=calculatefinal(maxminfinalroute,customerno-1)
     print("Cost for acs",acspartial+costacs+200000)
     print("Cost for ellitist",ellitistpartial+costellitist+200000)
     print("Cost for maxmin",maxminpartial+costmaxmin+200000)
